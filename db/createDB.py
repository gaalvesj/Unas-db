...
cursor.execute('''CREATE TABLE IF NOT EXISTS Tabela_UNAS (id INTEGER PRIMARY KEY, projeto TEXT, nome TEXT, idade TEXT, sexo TEXT, raca TEXT, bairro TEXT )''')
...
for arquivo in arquivos_excel:
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        cursor.execute("INSERT INTO Tabela_UNAS (projeto, nome, idade, sexo, raca, bairro) VALUES (?, ?, ?, ?, ?, ?)", (row[1], row[2].upper(), row[3], row[4], row[5], row[6]))
